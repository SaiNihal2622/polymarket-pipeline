...
def _run_async(coro):
    ...
    Works reliably from background threads (no event loop) by always
    creating a fresh loop via asyncio.run()."""
    try:
        return asyncio.run(coro)
    except RuntimeError:
        # If "event loop is closed" or similar, create a brand new loop
