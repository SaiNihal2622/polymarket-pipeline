"""Try create_market_order instead of limit order."""
import sys
for m in list(sys.modules.keys()):
    if 'py_order_utils' in m or 'py_clob_client' in m:
        del sys.modules[m]

import os
from dotenv import load_dotenv
load_dotenv()
from py_clob_client.client import ClobClient
from py_clob_client.clob_types import OrderArgs, OrderType, MarketOrderArgs
from eth_account import Account as EthAccount

priv = os.getenv('POLYMARKET_PRIVATE_KEY')
if not priv.startswith('0x'): priv = '0x'+priv
wallet = EthAccount.from_key(priv).address
proxy = 'https://vercel-proxy-nine-rose.vercel.app'

# Try FOK order type
print("--- FOK order ---")
try:
    c = ClobClient(host=proxy, key=priv, chain_id=137, funder=wallet)
    cr = c.derive_api_key()
    c.set_api_creds(cr)
    o = OrderArgs(price=0.10, size=0.50, side='BUY', token_id='74636610772409469817718475200152067076720965641263785464662938420699072982790')
    s = c.create_order(o)
    r = c.post_order(s, OrderType.FOK)
    print(f'SUCCESS: {r}')
except Exception as e:
    print(f'ERROR: {e}')

# Try market order
print("\n--- Market order ---")
try:
    c = ClobClient(host=proxy, key=priv, chain_id=137, funder=wallet)
    cr = c.derive_api_key()
    c.set_api_creds(cr)
    mo = MarketOrderArgs(amount=0.50, side='BUY', price=0.15, token_id='74636610772409469817718475200152067076720965641263785464662938420699072982790')
    s = c.create_market_order(mo)
    r = c.post_order(s, OrderType.FOK)
    print(f'SUCCESS: {r}')
except Exception as e:
    print(f'ERROR: {e}')